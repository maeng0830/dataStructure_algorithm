...
public class Heap<T> {
	...
	private final Comparator<? super T> comparator; // 객체 비교를 위한 필드
	...
	public void add(T target) {
		...
		siftUp(size + 1, target); // 타겟이 추가될 위치와, 타겟을 넘겨준다.
		size++; // 추가 후 size 증가
	}
	...
	public T remove() {
		...
		T result = array[1]; // 삭제된 노드를 반환하기 위한 임시 변수
		T target; // 타겟이 될 요소
		...
		array[size] = null; // 타켓 노드를 삭제한다.

		siftDown(1, target); // 삭제할 노드의 인덱스, 재배치될 타겟 노드를 넘겨준다.
